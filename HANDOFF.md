...
#### ⚠️ M3 未能验证的原因（2026-09-13 晚，机器级故障）

**本机所有仓颉 exe 启动即崩**（`0xC0000005`；事件日志里故障模块恒为 `msvcrt.dll`、偏移 `0x62f1e`）：
本项目 exe、轻舟框架 exe、`cjenv.exe`，连 1.0.5 与 1.1.3 两套 SDK 的产物都一样；
用"进 `main` 就写文件"的探针确认 **`main()` 根本没执行**。WER 报告里无任何第三方模块注入。
时间线：**21:40 重启后开始**——当日 13:58 时 M2 的 165 项冒烟测试还是全绿的。

完整排查与建议的处置顺序见 `server/API-NOTES.md` §5。**恢复后第一件事**：
在 `server/` 下跑 `.\build\club-server.exe test` 与 `tests\smoke.ps1`，确认 M3 通过。
