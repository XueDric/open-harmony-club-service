...
| M3 | Part 4 任务 8 个接口（`/tasks/mine` 服务端分组、`BLOCKER_REQUIRED`、软删除、`/tasks/lookup`、`client_token` 幂等） | ⚠️ **代码写完并编译通过，但运行验证被本机环境阻塞** |
| M4 | Part 5 课题 6 个接口（环形校验、深度 6、删除上提、O(n) 聚合） | 待排 |
| M5 | 打包部署 · 带 SAN 自签证书 · TLS 关卡 | **卡在服务器步骤 0**（架构 / 公网 IP / 端口） |

#### ⚠️ M3 未能验证的原因（2026-09-13 晚，**已定位到具体更新**）

**本机所有仓颉 exe 启动即崩**（`0xC0000005`；故障模块恒为 `msvcrt.dll`、偏移 `0x62f1e`，
...
当日 13:58 时 M2 的 165 项冒烟测试还是全绿的。用 WinSxS 里更新前后的两份 `msvcrt.dll`
（`10.0.26100.8875` vs `.9444`）逐字节对比，**`wcslen` 机器码完全相同**——
说明不是 msvcrt 被改坏，而是**运行时所处的环境变了**；`curl/certutil/where` 等同样用 CRT 的程序全部正常。
**即：Windows 更新 KB5124010 与仓颉 1.1.3 原生运行时不兼容。**

**处置**：优先**卸载 KB5124010**（预览更新，属可选更新）：设置 → Windows 更新 → 更新历史 →
卸载更新，重启后即可恢复。官方当前 **LTS=1.0.5、STS=1.1.3**，**没有更新的仓颉版本可升级**。
完整证据链与备选方案见 `server/API-NOTES.md` §5。

**恢复后第一件事**：在 `server/` 下跑 `.\build\club-server.exe test` 与 `tests\smoke.ps1`，确认 M3 通过。
