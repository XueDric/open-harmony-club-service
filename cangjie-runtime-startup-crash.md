...
| **我们的判断** | 仓颉原生运行时（DLL 构建于 **2025/7/30**）在该补丁级别上，于 `main()` 之前发生非法内存访问 |
| **进一步收窄** | 卸载 KB5124010 后，已加载模块中**唯一版本变化的是 `ntdll.dll`（9539 → 9278）**；msvcrt / ucrtbase / msvcp_win / KERNEL32 / KERNELBASE 全部未变 → 建议优先看**加载器与该版本运行时的交互** |
| **影响** | 本机**无法编译验证、无法运行、无法测试任何仓颉程序**；我们的服务端项目（仓颉 1.1.3 + 轻舟框架）被完全阻塞 |
| **已验证的规避方案** | ✅ **卸载 KB5124010 后立即恢复**（同机、同 SDK、同 msvcrt）：hello world 正常、单测 197 项全绿、HTTP 冒烟 220 项全绿（见 §9 受控对照） |
| **状态** | 我们已用规避方案恢复开发，但**该缺陷仍会影响所有安装此更新的仓颉用户**，故提交本报告 |
...
## 7. 我们的处置与**验证结果**（已按方案 1 处理，确认有效）

1. ✅ **已卸载 KB5124010 并重启（2026-09-13 22:18）→ 问题消失**，详见 §9 的受控对照。
...
6. 我们已把问题收窄到 **`ntdll.dll 10.0.26100.9539`**（见 §9）：在你们能复现的前提下，
   建议优先看**加载器（ntdll）与该版本仓颉运行时的交互**，而不是 msvcrt 本身。

---

## 9. 受控 A/B 验证：卸载该更新后立即恢复

卸载 KB5124010 并重启后（2026-09-13 22:18），**同一台机器、同一套仓颉 1.1.3、同一份 `msvcrt.dll`**，
结果完全反转：

| 指标 | 故障时 | 卸载 KB5124010 后 |
| --- | --- | --- |
| 系统 UBR | 26300.**9539** | 26300.**9445** |
| KB5124010 | 已安装 | **已不在**（`Get-HotFix` 查不到） |
| 最小 hello world | ❌ `0xC0000005`，无输出 | ✅ **`exit 0`，正常输出** |
| 我们的服务端 exe（约 3900 行仓颉） | ❌ `0xC0000005`，`main()` 未执行 | ✅ **单测 197 项全绿** |
| HTTP 冒烟测试（真实接口） | 无法启动 | ✅ **220 项全绿** |

### 9.1 关键收窄：唯一变化的已加载模块是 **`ntdll.dll`**

我们对崩溃进程加载的关键系统 DLL 做了卸载前后逐项对比：

| DLL | 故障时（UBR 9539） | 卸载后（UBR 9445） | 是否变化 |
| --- | --- | --- | --- |
| **`ntdll.dll`** | **10.0.26100.9539** | **10.0.26100.9278** | **✅ 变了** |
| `msvcrt.dll` | 7.0.26100.9444 | 7.0.26100.9444 | 未变 |
| `ucrtbase.dll` | 10.0.26100.9444 | 10.0.26100.9444 | 未变 |
| `msvcp_win.dll` | 10.0.26100.9444 | 10.0.26100.9444 | 未变 |
| `KERNEL32.DLL` | 10.0.26100.9278 | 10.0.26100.9278 | 未变 |
| `KERNELBASE.dll` | 10.0.26100.9278 | 10.0.26100.9278 | 未变 |

**这解释了 §4.3 的反常现象**：崩溃点所在的 `msvcrt!wcslen` 机器码在更新前后一字未改，
因为**真的与 msvcrt 无关**——变的是 **ntdll（Windows 加载器）**。
所以我们把怀疑范围收窄为：**ntdll 9539 与仓颉运行时启动路径的交互**。

> 附注：本机在**正常运行**时 `PendingFileRenameOperations` 也非空，因此该注册表项
> **不能**作为"系统更新未收尾"的判据；我们早前基于它的推测已被这次对照推翻，特此更正。

> 另附：卸载后我们复测了本项目的完整测试集（单测 197 + HTTP 冒烟 220），全部通过，
> 说明这确实是一次**环境问题**，与用户代码无关。
