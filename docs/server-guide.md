...
| **M7 第二轮评审** | `docs/code-review.md` 第二轮：独立复验第一轮 24 条（全部确认修复）+ **9 条新发现**（N-1…N-9） | ✅ **已完成**——9 条全部处理（N-8 按约定不改）：N-1（落地页转义）/ N-2 / N-3 / N-4 / N-5（`idem` 补校验）/ **N-6（注册节流改按客户端 IP + 递增退避，未新增接口 → 仍是 39 / 39）** / **N-7（不可作用于同权或更高权的人，已推广到改名 / 改角色 / 禁用）** / N-9（CSP），每条都带"回退即变红"的回归断言。当前基线 **297 / 343 / 22 全绿** |
...
```powershell
# 单测：时间/历法、口令哈希、权限矩阵（含档位规则、N-7「不可作用于同权/更高权者」）、注册节流（N-6 按 IP + 退避）、落盘往返与坏记录拒绝（含 N-5）、视图/分页、任务分组、课题树与聚合（297 项）
.\build\club-server.exe test

# 冒烟测试：真实 HTTP、状态码、错误码、权限边界（含 H-1 同部门接管与 N-7 同权重置/降级/禁用）、幂等、环形校验、删除上提、审计日志、落地页转义与 CSP、注册节流（N-6）、重启持久性（343 项）
